  ...
  const handleSaveStartDate = async () => {
    ...
    if (!nextValue) {
      toast({
        ...
        description: "Ange ett giltigt startdatum.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const { error: updateError } = await supabase
        ...
        .eq("key", "safety_start_date");

      if (updateError) throw updateError;

      toast({
        title: "Uppdaterat",
        description: "Startdatumet är uppdaterat.",
      });
    } catch (err: any) {
      toast({
        title: "Fel",
        description: err?.message ?? "Kunde inte spara datumet.",
        variant: "destructive",
      });
    } finally {
      setSaving(false);
          ...
          <div className="mt-6 rounded-xl border bg-background p-4">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <Input
                ...
                className="max-w-xs"
              />
              <Button onClick={handleSaveStartDate} disabled={saving}>
                {saving ? "Sparar..." : "Spara startdatum"}
              </Button>
            </div>
            <p className="mt-2 text-xs text-muted-foreground">
              Format: ÅÅÅÅ-MM-DDTHH:MM (t.ex. 2025-11-18T00:00)
            </p>
          </div>
        )}
      </CardContent>
